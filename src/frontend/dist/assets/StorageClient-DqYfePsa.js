...
import { e as encode, i as isV3ResponseBody, T as Text } from "./index-yvaPo1GN.js";
...
class StorageClient {
  constructor(actor, bucket, storageGatewayUrl, backendCanisterId, projectId, agent) {
  ...
  async putFile(path, file, onProgress) {
    if (!path) {
    ...
    const url = `${this.storageGatewayClient.getStorageGatewayUrl()}/${GATEWAY_VERSION}/blob/?blob_hash=${encodeURIComponent(hashString)}&owner_id=${encodeURIComponent(this.backendCanisterId)}&project_id=${encodeURIComponent(this.projectId)}`;
    return { path, hash: hashString, url };
  }
  async listObjects() {
    return await this.actor.listFileReferences();
  }
  async getDirectURL(path) {
    if (!path) {
